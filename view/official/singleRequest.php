<?php 
    ...
    $currentUser=getUserById($_SESSION['id']);
    $currentUserType=$currentUser['user_role'];
    if(str_contains($currentUserType,'lab_asistance')){
        $displayOrNot='';
    }
    else{
        $displayOrNot='d-none';
    ...
    $userType=$user['user_role'];
        if($userType=='department_head' || $userType=='exam_controller'){
            $action=" <button class='btn btn-success btn-sm btn-gap mx-2' onclick='approveRequest($request[id])'>Approve</button>
            <button class='btn btn-danger btn-gap' onclick='declineRequest($request[id])'>Decline</button>";
        }
        else if($userType=='liabarian' || $userType=='hall_provost' || $userType=='finace_section'){
            $action="<button class='btn btn-success btn-sm btn-gap mx-2' onclick='approveRequest($request[id])'>Approve</button>
            <button class='btn btn-danger btn-gap' data-bs-toggle='modal' data-bs-target='#declineModal1'>Decline</button>";
        }
        else{
            $action="<button class='btn btn-success btn-sm btn-gap mx-2' onclick='approveRequest($request[id])'>Approve</button>
            <button class='btn btn-danger btn-gap' data-bs-toggle='modal' data-bs-target='#declineModal1'>Decline</button>";
...
<div class="container-fluid d-flex justify-content-center">
  ...
  <br><br><br><br><br>
        <div class="container-fluid mt-5">
            ...
            <div id="labClearencce" class="<?=$displayOrNot?>">
