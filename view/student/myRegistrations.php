<?php 
    ...
    $allRegistrations=getAllRegistration();
    $registrations=[];

    foreach ($allRegistrations as $singleRegistration) {
        if($singleRegistration['student_id']==$_SESSION['id']){
            array_push($registrations, $singleRegistration); 
        }
    }
        ...
        if(sizeof($registrations)==0){
            ...
            foreach ($registrations as $registration) {
                ...
                echo "<tr>
                            ...
                            <td>
                            <button class='btn btn-danger btn-sm btn-gap' onclick='deleteRegistration($registration[id])'>Delete</button>
                            <button class='btn btn-primary btn-sm' onclick='viewSingleRegistrationStudent($registration[id])'>View</button>
                            </td>
                        </tr>";
